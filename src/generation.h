...
/**
 * Defines a value space for any errors which may be
 * returned from the public interface of generation.c
 */
typedef enum NCM_GENERATION_STATUS_ENUM {
	 ...
	 * Either it is "full" in the sense that no more coded packets can
	 * be added or, in the context of a `DESTINATION`, it is fully drained,
	 * meaning all packets were already decoded.
	 */
	GENERATION_FULLY_TRAVERSED = -40702,
	/**
	 * The given packet had unexpected size
	 * (probably exceeding the defined maximum pdu of the coding matrix).
	 */
	GENERATION_PACKET_TOO_LARGE = -40703,
	/**
	 * There wasn't enough (linear independent aka. "new")
	 * information in order to decode the next frame.
	 */
...
struct generation_packet_counter {
	...
	int data;
	// counts sent acknowledgment packets
	int ack;
...
enum GENERATION_EVENT {
	/**
	 * Event triggered when the generation should be queued
	 * for a ACK transmission.
	 * Event doesn't pass a `result` pointer.
	 */
	GENERATION_EVENT_ACK,
	/**
	 * Event triggered when the generation wants to send out a
	 * new coded packet.
	 * Event doesn't pass a `result` pointer.
	 */
	GENERATION_EVENT_ENCODED,
	/**
	 * Event triggered before the generation is cleared and its
	 * generation sequence number is advanced.
	 * Event doesn't pass a `result` pointer.
	 */
	GENERATION_EVENT_RESET,
	/**
	 * Event triggered when the generation wants to retrieve the current
	 * session redundancy, calculated using the current link quality.
	 * Event passes a `double` pointer to the `result` parameter
	 * to which the result must be written.
	 */
 ...
 * @param result - Some events expect a result to be returned.
 * 	The result must be written into the `result` pointer.
 *      For events not expecting a result a NULL pointer is passed.
 */
typedef void (*generation_event_handler)(generation_t* generation,
 ...
 *
 * @param generation_list - The `list_head` to the generation list,
 * 	this generation should be inserted into.
 * @param session_type - The `SESSION_TYPE` of the session,
 * 	the created generation will be added to.
 ...
 * @param event_handler - Pointer to the `generation_event_handler`.
 * 	Can be NULL to not register an event handler.
 * @param event_data - Pointer passed to the `data` argument of an
 * 	`generation_event_handler` on every triggered event.
...
/**
 * Adds a source frame (e.g. received from the OS) to the next available
 * generation in the provided generation list.
 *
 * @param generation_list - The `generation` list, to choose the next available
 * 	generation from.
 ...
 * @param generation_sequence - Pointer to which the `u16` generation sequence
 * 	number is written to, if (and ONLY if) the function returns successfully.
 ...
 * @return Returns an `NCM_GENERATION_STATUS`.
 *  Note: parameters `generation_sequence` and `length_encoded` are
 *  	only written to on a `GENERATION_STATUS_SUCCESS`.
 */
NCM_GENERATION_STATUS
 ...
 *
 * @param generation_list - The list to iterate over the next available
 * 	decodable frame.
 * @param max_length - The maximum size of the provided `buffer`.
 * @param buffer - Pointer to the buffer to write result into.
 * @param length_decoded - If a new decodable frame is available,
 * 	its length is written to this pointer.
 *  	Note: This pointer is only written to,
 *  	if returned status equals to `GENERATION_STATUS_SUCCESS`
...
/**
 * Fills in the fields of a ack payload struct according
 * to the data in the generations.
 *
...
/**
 * Returns the first sequence number in the generation_window
 * (referred to as window_id)
 */
u16
...
/**
 * Returns the amount of free space inside the coding matrix
 * for the given `generation_t`.
 *
 * Equivalent formulations:
 *  - The amount of source frames yet to be sent.
 *  - The amount of encoded frames yet to received (later decoded/recoded).
 *
